...
public class Worker : BackgroundService
{
    ...
    public Worker(ILogger<DlqWorker> logger, ServiceBusClient client, IOptions<ServiceBusOptions> opt)
    {
        _logger = logger;
        _client = client;
        _opt = opt.Value;
    ...
    private async Task OnSessionMessageAsync(ProcessSessionMessageEventArgs arg)
    {
        ...
        try
        {
            var received = msg.Body.ToObjectFromJson<MqObjectItem>();
            _logger.LogInformation("TopicId:{TopicId}, TopicName:{topicName}, MetadataId:{metadataId}, DriveId:{driveId}, ItemId:{itemId}, IsFolder:{isFolder}", 
                received.TopicId, received.TopicName, received.MetadataId, received.DriveId, received.ItemId, received.IsFolder);
