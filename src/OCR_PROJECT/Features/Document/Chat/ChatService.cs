...
public class ChatService: DiaExecuteServiceBase<ChatService, DiaDbContext,  ChatRequest, DocumentChatResult>, IChatService
{
    ...
    public override async Task<DocumentChatResult> ExecuteAsync(ChatRequest request)
    {
        ...
        for (var attempt = 1; attempt <= maxRetries; attempt++)
        {
            try
            {
                var thread = await this.dbContext.Threads.Where(m => m.Id == request.ThreadId)
                    .FirstOrDefaultAsync();
                ...
                await this.dbContext.QuestionResearches.AddRangeAsync(planResult.researches);
                
                var previous = await this.dbContext.Questions.AsNoTracking()
                ...
                {
                    isContextSwitch =
                        await _questionContextSwitchService.ExecuteAsync(
                ...
                var cleanCitations = (result.Citations ?? Enumerable.Empty<PageCitation>())
                    ...
                    .Take(50)
                    .Select(c => new DOCUMENT_CITATION { File = c.File, Page = c.Page })
                    .ToList();
                
                var answer = new DOCUMENT_ANSWER()
                {
    ...
    private async Task<(List<DOCUMENT_QUESTION_RESEARCH> researches, QueryPlan plan)> GenerateQueryPlanAndResearches(DOCUMENT_THREAD documentThread, DOCUMENT_QUESTION documentQuestion, string question)
    {   
        var messages = new List<ChatMessage>()
        {
        ...
        var currentHits = new List<DOCUMENT_QUESTION_RESEARCH>();
        ...
        foreach (var doc in filtered.Select(hit => hit.Doc))
        {
            ...
            currentHits.Add(new DOCUMENT_QUESTION_RESEARCH
            {
                Id = Guid.NewGuid(),
                ChunkId = cid,
        ...
        var addList = currentHits
            ...
            .Select(hit => new DOCUMENT_QUESTION_RESEARCH
            {
    ...
    private async Task<ChatResult> AskFromGpt(IEnumerable<DOCUMENT_QUESTION_RESEARCH> items, Guid threadId, string question, bool isShift)
    {
        ...
        {
            //TODO: 날짜 정렬로 변경해야...
            var history = await this.dbContext.Questions
                .AsNoTracking()
